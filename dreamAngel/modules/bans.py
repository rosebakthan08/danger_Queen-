...
from dreamAngel.modules.disable import DisableAbleCommandHandler
from dreamAngel.modules.helper_funcs.chat_status import (
    bot_admin,
    can_restrict,
    connection_status,
    is_user_admin,
...
from dreamAngel.modules.helper_funcs.extraction import extract_user_and_text
from dreamAngel.modules.helper_funcs.string_handling import extract_time
from dreamAngel.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Bring an order from dreamAngel association to fight a Demon 👽disaster."
            )
            return log_message
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from dreamAngel association to fight a Tiger 🤤disaster."
            )
            return log_message
